...
typedef int32_t (*read_block_type_t)(struct fs_node*, uint32_t, uint32_t,
                                     uint8_t*);
typedef int32_t (*write_block_type_t)(struct fs_node*, uint32_t block_number,
...
typedef struct fs_node {
  ...
  uint32_t inode;   // This is device-specific - provides a way for a filesystem
                    // to identify files.
  uint32_t length;  // Size of the file, in bytes.
  uint32_t impl;    // An implementation-defined number.
  read_type_t read; // These typedefs are just function pointers. We'll define
                    // them later!
  write_type_t write;
  open_type_t open;
  close_type_t close;
...
struct dirent // One of these is returned by the readdir call, according to
              // POSIX.
{
