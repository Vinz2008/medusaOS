...
void draw_char_size(fb_t fb, int size_mult, char c, int x, int y,
                    uint32_t col) {
  log(LOG_SERIAL, false, "draw character: %c x : %d y : %d col : %d\n", c, x, y,
      col);
#if GUI_MODE
    ...
    for (int j = 0; j < 8; j++) {
      if (offset[i] & (1 << j)) {
        ...
        for (int add = 0; add < size_mult; add++) {
          draw_pixel(fb, (x + 8 - j) * size_mult + add,
                     (y + i) * size_mult + add, col);
        }
      }
    }
  }
...
void draw_string_size(fb_t fb, int size_mult, const char* str, int x, int y,
                      uint32_t col) {
  size_t len = strlen(str);
  for (size_t i = 0; i < len; i++) {
