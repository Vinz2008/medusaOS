...
int16_t pci_config_read(uint8_t bus, uint8_t slot, uint8_t func,
                        uint8_t offset) {
  ...
  outb(CONFIG_ADDR, address);
  return (uint16_t)((inb(CONFIG_DATA) >> ((offset & 2) * 8)) & 0xFFFF);
}
...
static void config_write(uint8_t bus, uint8_t slot, uint8_t func,
  ...
  uint32_t address = (uint32_t)(((uint32_t)bus << 16) | ((uint32_t)slot << 11) |
                                ...
                                ((uint32_t)0x80000000));
  outb(CONFIG_ADDR, address);
  outb(CONFIG_DATA, (uint16_t)value);
}
...
static inline void init_dev(pci_device_t* dev, uint8_t bus, uint8_t slot,
                            uint8_t func) {
...
pci_device_t pci_find_any(uint8_t class_code, uint8_t subclass_code,
                          int8_t interface_value) {
  pci_device_t dev;
...
pci_device_t pci_find(unsigned int vendor_id, unsigned int device_id) {
  ...
  for (uint8_t bus = 0; bus < 5; ++bus) {
    for (uint8_t slot = 0; slot < 32; ++slot) {
      for (uint8_t func = 0; func < 8; ++func) {
        if (read_devid(bus, slot, func) == device_id &&
