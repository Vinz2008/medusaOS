...
void init_pmm(uint32_t addr) {
  ...
  if ((uint32_t)&KERNEL_END > KERNEL_END_MAP) {
    log(LOG_SERIAL, false, "The kernel is too large for its initial mapping");
    abort();
  }
  ...
  log(LOG_SERIAL, false, "memory stats: available: %d MiB\n", available >> 20);
  log(LOG_SERIAL, false, "unavailable: %d KiB\n", unavailable >> 10);
  log(LOG_SERIAL, false, "taken by modules: %d MiB\n",
      (kernel_end - (uintptr_t)&KERNEL_END_PHYS) >> 20);
}

/* Returns the number of bytes allocated by the PMM.
 */
uint32_t pmm_used_memory() { return used_blocks * PMM_BLOCK_SIZE; }

/* Returns the number of free bytes the PMM started with.
 */
uint32_t pmm_total_memory() { return mem_size; }
...
uintptr_t pmm_alloc_page() {
  if (max_blocks - used_blocks <= 0) {
    log(LOG_SERIAL, false, "kernel is out of physical memory!");
    abort();
  }
...
uint32_t mmap_test(uint32_t bit) { return bitmap[bit / 32] & NTHBIT(bit % 32); }
