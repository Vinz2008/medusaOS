...
fs_node_t* get_root_devfs() { return dev_root; }
...
struct dirent* devfs_readdir(struct fs_node* node, uint32_t index) {
  ...
  if (index >= device_list->used) {
    if (initrd_readdir_call) {
      // put to 0 to reset index for if last call to readdir was to an other
      // filesystem
      index = 0;
    } else {
      return NULL;
...
device_list[n_devices].name); n_devices++;
}*/
...
fs_node_t* devfs_register_device(Device* device) {
  ...
  log(LOG_SERIAL, false, "after adding device_list[%d] : %s\n",
      device_list->used - 1,
      ((fs_node_t*)device_list->list[device_list->used - 1].data)->name);
  return device_node;
}
...
void devfs_initialize() {
  ...
  dev_root->mask = dev_root->uid = dev_root->gid = dev_root->inode =
      dev_root->length = 0;
