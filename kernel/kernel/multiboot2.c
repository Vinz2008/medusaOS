...
static const char* tag_table[] = {
    "TAG_END",   "TAG_CMDLINE", "<unknown>",  "TAG_MODULE",
    "TAG_MEM",   "TAG_BOOTDEV", "TAG_MEMMAP", "TAG_VBE",
    "TAG_FB",    "<unknown>",   "TAG_APM",    "<unknown>",
    "<unknown>", "<unknown>",   "TAG_RSDP1",  "TAG_RSDP2",
};
...
void mb2_print_tags(mb2_t* boot) {
  if (boot->total_size <= sizeof(mb2_t)) {
    log(LOG_SERIAL, false, "no tags given");
    return;
  }
  ...
  do {
    ...
    log(LOG_SERIAL, false, "%s (%d): %d bytes", tag_name, tag->type, tag->size);
