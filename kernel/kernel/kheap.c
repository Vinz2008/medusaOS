...
static int32 find_smallest_hole(uint32_t size, uint8_t page_align, heap_t *heap)
{
...
void free(void *p, heap_t *heap){
	...
	if (test_header->magic == HEAP_MAGIC && test_header->is_hole) {
		...
		test_footer	= (footer_t*) ( (uint32_t)test_header + test_header->size - sizeof(footer_t) );	// Rewrite it's footer to point to our header.
		footer		= test_footer;
